...
var DefaultViewPageIndicator = require('./ViewPageIndicator');
var deviceWidth = Dimensions.get('window').width;
var ViewPagerDataSource = require('./ViewPagerDataSource');


/**
 *   项目参考 地址     https://github.com/race604/react-native-viewpager
 */
...
var ViewPager = React.createClass({
  ...
  componentWillMount() {
    ...
    var release = (e, gestureState) => {
      ...
      var step = 0;
      if (relativeGestureDistance < -0.2 || (relativeGestureDistance < 0 && vx <= -1e-6)) {
        step = 1;
      } else if (relativeGestureDistance > 0.2 || (relativeGestureDistance > 0 && vx >= 1e-6)) {
        step = -1;
      }
  ...
  _startAutoPlay() {
    if (!this._autoPlayer) {
      this._autoPlayer = this.setInterval(
        () => {
          if(this.props.dataSource.getPageCount()>0){
            this.movePage(1);
          }
        },
        5000
